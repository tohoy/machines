    ...
    def run(self):
        while not self.baker.quit:
            ...
            self.screen.addstr(
                ...
                2,
                "Watchdog TTL: {0:.0f}  ".format(self.watchdog.time_to_live),
            )
            self.screen.addstr(
                text_line := text_line + 1,
                2,
                "Watchdog Timer: {0:.1f}".format(time.time() - self.watchdog.timer),
            )
            self.screen.addstr(
                text_line := text_line + 1,
                2,
                "Watchdog safe: {}    ".format(self.watchdog.watchdog_safe),
            )

            self.screen.addstr(text_line := text_line + 2, 2, 'Current channel status:')
            text_line = text_line + 1
            for i in range(1, 7):
                self.screen.addstr(text_line, 6 * i, str(wp.digitalRead(i)))

            self.screen.addstr(text_line := text_line + 3, 2, "Channel duty cycles")
            text_line = text_line + 1
            for i in range(1, 7):
                self.screen.addstr(
                    text_line, 6 * i, str(round(self.baker.dutycycles[i - 1], 2))
                )
            ...
            if self.baker.run_ramp == True:
                ...
                self.screen.addstr(
                    text_line := text_line + 1, 2, f'Currently requested dutycycles:'
                )
                self.screen.addstr(
                    text_line := text_line + 1, 7, str(self.baker.ramp.present())
                )
            else:
                text_line = text_line + 1
            ...
            self.screen.addstr(
                text_line := text_line + 3, 2, "l: load ramp from ramp.py, "
            )
            self.screen.addstr(
                text_line := text_line + 1,
                2,
            ...
            self.screen.addstr(
                text_line := text_line + 1, 2, "s: forcefully stop the ramp       "
            )
            self.screen.addstr(text_line := text_line + 1, 2, "q: quit program       ")
            n = self.screen.getch()

            self.screen.addstr(
                text_line := text_line + 1, 2, "Last key stroke: {}     ".format(n)
            )
...
def main(stdscr):
    ...
    datasocket = DateDataPullSocket(
        'stm312_bakeoutbox', ['stm312_bakeoutbox'], timeouts=[1.0]
    )
    datasocket.start()
...
if __name__ == '__main__':
    curses.wrapper(
        main
    )  # the wrapper helps ensure that "If the application raises an exception, this function will restore the terminal to a sane state before re-raising the exception and generating a traceback"
